 ...
 *     all the input arrays have a shape matching their data shape, and then
...
struct squeeze_first_axis_if_single_input_and_scalarize<N, MappingModel>
  : out_arr_transform_base<N, MappingModel>
{
    typedef squeeze_first_axis_if_single_input_and_scalarize<N, MappingModel>
            type;

    inline static int
    apply(ndarray & out_arr, BOOST_PP_ENUM_PARAMS(N, ndarray const & in_arr_))
    {
        if(squeeze_first_axis_if_single_input<N, MappingModel>::apply(out_arr, BOOST_PP_ENUM_PARAMS(N, in_arr_)))
        {
